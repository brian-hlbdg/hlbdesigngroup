      ...
      <main>
        <div class="w-full mx-auto px-8 md:px-12 pt-16 flex flex-col md:flex-row">
          <div class="flex-1 md:pr-8 lg:pr-12 pt-12 md:max-w-[75%] lg:max-w-[80%]">
            <div>
              <h1 id="hobbies-heading" class="text-4xl md:text-5xl font-bold leading-tight mb-8 text-gray-900 dark:text-white transition-colors" phx-hook="FadeIn">
                Hobbies & Interests
              </h1>

              <p id="hobbies-intro" class="text-xl text-gray-600 dark:text-gray-300 max-w-prose mb-12 transition-colors" phx-hook="FadeIn">
                ...
                <%= for category <- @hobby_categories do %>
                  <button
                    ...
                    class={"px-5 py-2 rounded-full text-sm font-medium transition-colors whitespace-nowrap " <>
                      ...
                      do: "bg-primary dark:bg-primary text-white dark:text-white",
              ...
              <div id="category-content" phx-hook="FadeIn">
                <h2 class="text-2xl font-light mb-4 text-gray-800 dark:text-gray-200 transition-colors"><%= @selected_category.title %></h2>
                <p class="text-gray-600 dark:text-gray-300 mb-8 transition-colors"><%= @selected_category.description %></p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <%= for {image, i} <- Enum.with_index(@selected_category.images) do %>
                    <div id={"image-#{@selected_category.id}-#{i}"} class="overflow-hidden rounded-sm shadow-sm" phx-hook="ImageHover">
                      ...
                      />
                      <div class="p-3 bg-gray-50 dark:bg-gray-800 transition-colors">
                        <p class="text-sm text-gray-600 dark:text-gray-300 transition-colors"><%= image.caption %></p>
                      </div>
                    </div>
                  <% end %>
                </div>

                <div id="category-description" class="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700 transition-colors">
                  <h3 class="text-xl font-medium mb-4 text-gray-800 dark:text-gray-200 transition-colors">Why <%= String.downcase(@selected_category.title) %> matters to me</h3>
                  <p class="text-gray-600 dark:text-gray-300 transition-colors mb-4">
