...
defmodule PortfolioWeb.HomeLive do
  ...
  def render(assigns) do
    ~H"""
    <div id="home-page-container" phx-hook="PageTransition" class="min-h-screen bg-white max-w-full overflow-hidden">
      ...
      <div class="w-full fixed bottom-0 md:hidden">
        <nav class="flex justify-around py-4 bg-white border-t border-gray-200">
          <.link navigate={~p"/"} class="text-xs text-[rgb(211,84,0)] flex flex-col items-center">
            <span class="material-icons text-lg">home</span>
            <span>Home</span>
          </.link>
          <.link navigate={~p"/projects"} class="text-xs text-gray-500 flex flex-col items-center">
            <span class="material-icons text-lg">work</span>
            <span>Work</span>
          </.link>
          <.link navigate={~p"/process"} class="text-xs text-gray-500 flex flex-col items-center">
