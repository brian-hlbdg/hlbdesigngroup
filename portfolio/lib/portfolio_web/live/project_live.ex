      ...
      <main>
        <div class="w-full mx-auto px-8 md:px-12 pt-16 flex flex-col md:flex-row">
          <div class="flex-1 md:pr-8 lg:pr-12 pt-12 md:max-w-[75%] lg:max-w-[80%]">
            <div>
              <h1 id="work-heading" class="text-4xl md:text-5xl font-bold leading-tight mb-8 text-gray-900 dark:text-white" phx-hook="FadeIn">
                Selected Work
              </h1>

              <div id="featured-work" class="mt-16" phx-hook="FadeIn">
                <h2 class="text-2xl font-light mb-6 text-gray-800 dark:text-gray-100">Featured Projects</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
                  <%= for project <- Enum.filter(@projects, & &1.featured) do %>
                    <div class="project-card border-t border-gray-200 dark:border-gray-700 pt-6" id={"project-#{project.id}"} phx-hook="ImageHover">
                      ...
                      <div class="flex flex-col space-y-2">
                        <h3 class="text-xl font-medium dark:text-white"><%= project.title %></h3>
                        <p class="text-gray-600 dark:text-gray-300"><%= project.description %></p>
                        <div class="flex justify-between items-center mt-4">
                          <div class="text-sm text-gray-500 dark:text-gray-400"><%= project.client %>, <%= project.year %></div>
                          <.link navigate={~p"/projects/#{project.id}"} class="text-sm text-gray-600 hover:text-primary dark:text-gray-300 dark:hover:text-primary-dark transition-colors group inline-flex items-center">
                            View Project
              ...
              <div id="other-work" class="mt-16" phx-hook="FadeIn">
                <h2 class="text-2xl font-light mb-6 text-gray-800 dark:text-gray-100">Additional Projects</h2>
                <div class="divide-y divide-gray-200 dark:divide-gray-700">
                  <%= for project <- Enum.filter(@projects, & !&1.featured) do %>
                    <div class="project-item py-6" id={"project-list-#{project.id}"}>
                      <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                        <div class="md:w-2/3">
                          <h3 class="text-xl font-medium mb-2 dark:text-white"><%= project.title %></h3>
                          <p class="text-gray-600 dark:text-gray-300 mb-4 md:mb-0"><%= project.description %></p>
                        </div>
                        <div class="flex flex-col md:items-end">
                          <div class="text-sm text-gray-500 dark:text-gray-400 mb-2"><%= project.client %>, <%= project.year %></div>
                          <.link navigate={~p"/projects/#{project.id}"} class="text-sm text-gray-600 hover:text-primary dark:text-gray-300 dark:hover:text-primary-dark transition-colors group inline-flex items-center">
                            View Project
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
