      ...
      <main>
        <div class="w-full mx-auto px-8 md:px-12 pt-16 flex flex-col md:flex-row">
          <div class="flex-1 md:pr-8 lg:pr-12 pt-12 md:max-w-[75%] lg:max-w-[80%]">
            <div>
              <h1 id="process-heading" class="text-4xl md:text-5xl font-bold leading-tight mb-8" phx-hook="FadeIn">
                My Design Process
              </h1>

              <p id="process-intro" class="text-xl text-gray-600 max-w-prose mb-12" phx-hook="FadeIn">
              ...
              <div class="hidden md:flex justify-between mb-16 border-b border-gray-200 pb-4">
                <%= for step <- @process_steps do %>
                  <button
                    phx-click="set_active_step"
                    phx-value-step={step.id}
                    class={"text-sm font-medium transition-colors " <> if @active_step == step.id, do: "text-[rgb(211,84,0)]", else: "text-gray-600 hover:text-[rgb(211,84,0)]"}>
                    <%= step.title %>
                  </button>
                <% end %>
              </div>
                ...
                <%= for {step, index} <- Enum.with_index(@process_steps) do %>
                  <div
                    ...
                    <div class="flex items-start mb-6">
                      <div class="flex-grow">
                        <h2 class="text-2xl font-medium mb-2"><%= step.title %></h2>
                        <p class="text-gray-600 mb-4"><%= step.description %></p>
                      </div>
                      <div class="text-4xl font-light text-gray-200 ml-4"><%= index + 1 %></div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                        <h3 class="text-lg font-medium mb-4">Key Activities</h3>
                        <ul class="space-y-2">
                      ...
                      <div>
                        <h3 class="text-lg font-medium mb-4">Tools & Methods</h3>
                        <div class="flex flex-wrap gap-2">
                          <%= for tool <- step.tools do %>
                            <span class="text-sm px-3 py-1 bg-gray-100 rounded-full"><%= tool %></span>
                          <% end %>
                        </div>
                      </div>
                    </div>

                    <div class="flex justify-between mt-12 pt-6 border-t border-gray-200 md:hidden">
                      <%= if index > 0 do %>
                        <button
                          phx-click="set_active_step"
                          phx-value-step={Enum.at(@process_steps, index - 1).id}
                          class="text-sm text-gray-600 hover:text-[rgb(211,84,0)] transition-colors group inline-flex items-center">
                      ...
                      <div class="flex items-start gap-16">
                        <div class="w-1/2">
                          <div class="flex items-start mb-6">
                            ...
                            <div class="text-5xl font-light text-gray-200 ml-4"><%= index + 1 %></div>
                          </div>
              ...
              <div id="process-cta" class="mt-16 pt-12 border-t border-gray-200" phx-hook="FadeIn">
                <h2 class="text-2xl font-light mb-6">Let's Work Together</h2>
